...
lsblk -e 7,11
echo ""
read -p "${bold}Enter the disk to partition (e.g., sda or nvme0n1): ${normal}" disk
base_disk=$(validate_input "$disk" "Enter the disk to partition: ")
cfdisk "/dev/$base_disk"
...
if [[ "$base_disk" =~ ^nvme ]]; then
    efi_part="/dev/${base_disk}p${efi_part}"
    btrfs_part="/dev/${base_disk}p${btrfs_part}"
    if [[ "$created_swap" == "yes" ]]; then
        swap_part="/dev/${base_disk}p${swap_part}"
    fi
else
    efi_part="/dev/${base_disk}${efi_part}"
    btrfs_part="/dev/${base_disk}${btrfs_part}"
    if [[ "$created_swap" == "yes" ]]; then
        swap_part="/dev/${base_disk}${swap_part}"
    fi
fi
...
mkdir -p /mnt/{boot/efi,var/cache,var/log,tmp,home/.snapshots,.snapshots}
...
base_packages="base linux-zen linux-zen-headers linux-firmware btrfs-progs grub grub-btrfs efibootmgr os-prober networkmanager nano git neofetch zsh zsh-completions zsh-autosuggestions openssh man sudo htop btop snapper"
